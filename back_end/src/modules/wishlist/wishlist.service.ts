...
export class WishlistService {
  ...
  async findWishlistByUser(userId: string) {
    ...
    items.map((wishlist: any) => {
      const validDiscount = wishlist.product.discounts.find(
        (discount: any) =>
      ...
      if (validDiscount) {
        const listDetailProducts = wishlist.product.detailProducts;

        listDetailProducts.map((detailProduct: any) => {
          const discountPrice = validDiscount
            ? detailProduct.price -
              (detailProduct.price * validDiscount.discount) / 100
            : detailProduct.price;
          detailProduct['finalPrice'] = discountPrice;
        });
      }
      return wishlist;
    });
